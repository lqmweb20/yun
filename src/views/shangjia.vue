<template>
	<div id="shangjia">
		<div class="p30  bgWhite mb30">
			<div class="ming pb30">
				<div class="font28 flex flex-jb">
					<p> 粥品香坊(大运村)</p> 
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"  viewBox="0 0 768 768" fill="red" class="go">
						<g id="icomoon-ignore">
						...
						<path d="M384 682.5l-46.5-42c-165-150-273-247.5-273-369 0-99 76.5-175.5 175.5-175.5 55.5 0 109.5 27 144 67.5 34.5-40.5 88.5-67.5 144-67.5 99 0 175.5 76.5 175.5 175.5 0 121.5-108 220.5-273 370.5z"></path>
					</svg>
				</div>
				<div class="flex flex-jb mt15">
					<div class="flex flex-ac font20 brown">
						<div class="flex">
							<img :src="(index+1)<=parseInt(shangjia.score)?xinxin:xinxin1" alt="" v-for="item,index in 5" :key="index" class="xinxin">
						</div>
						（661）
						<span>月销{{shangjia.sellCount}}</span>
					</div>
					<div class="flex flex-c  flex-ac brown font20">
						收藏
					</div>
				</div>
			</div>

			<div class="pt30 flex peisong ">
				<div class="flex-ac flex-jc flex flex-c">
					<p class="font20 gray">起送价格</p>
					<h1 class="font45">{{shangjia.minPrice}}  <small>元</small> </h1>
				</div>
				<div class="flex-ac flex-jc flex flex-c">
					<p class="font20 gray">商家配送</p>
					<h1 class="font45">{{shangjia.deliveryPrice}}<small>元</small> </h1>
				</div>
				<div class="flex-ac flex-jc flex flex-c bornone">
					<p class="font20 gray">平均配送时间</p>
					<h1 class="font45">{{shangjia.deliveryTime}}<small>分钟</small> </h1>
				</div>
			</div>
		</div>
	  
		<div class="p30  bgWhite mb30">
			<h3 class="font28 mb15">公告活动</h3>
			<p class="font26 red pb30">{{shangjia.bulletin}}</p>

			<ul class="gong">
				<li v-for="item,index in shangjia.supports" :key="index" class="font24 ptb30 flex flex-ac">
					<img src="../assets/img/decrease_3@2x.png" alt="" class="mr10 jian">
					{{item.description}}
				</li>
			</ul>
		</div>
		
		<div class="shijing p30  bgWhite mb30">
			<h3 class="font28 mb15">商家实景</h3>
			<div>
				<ul class="flex flex-ac">
					<li v-for="item,index in shangjia.pics" :key="index" class="mr15" >
						<img :src="item" alt="">
					</li>
				</ul>
			</div>
		...
		<div class="p30 bgWhite info">
			<h1 class="font28 mb15">商家信息</h1>
			<ul>
				<li v-for="item,index in shangjia.infos" :key="index" class="font24 ptb30">
					{{item}}
				</li>
			</ul>
		</div>
...
export default {
    mounted(){
      this.axios.get("data/data.json").then(d=>this.shangjia=d.data.seller);
...
.peisong{
  div{
    flex: 1;
    border-right: 0.01rem solid rgb(92, 90, 90);
  }
}
.peisong>.bornone{
...
.gong{
  li{
    border-top: 0.01rem solid rgb(92, 90, 90);
  }
}
.shijing{
	overflow: hidden;
